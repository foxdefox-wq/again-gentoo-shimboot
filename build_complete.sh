...
if [ -z "$kernel_ver" ]; then
  case "$board" in
    dedede)
      ...
      ;;
    octopus)
      kernel_ver="4.14.3137"
      ;;
    zork)
      kernel_ver="5.4.85"
      ;;
    hatch)
      kernel_ver="4.19.3369"
      ;;
    grunt)
      kernel_ver="4.19.3028"
      ;;
    nissa)
      kernel_ver="5.4.85"
      ;;
    snappy)
      kernel_ver="4.14.2419"
      ;;
    *)
      kernel_ver="auto"
      ;;
...
if [ "$kernel_ver" != "auto" ]; then
  print_info "using kernel version: $kernel_ver"
fi
...
final_image="$data_dir/shimboot_${distro}_${board}.bin"
rm -rf $final_image
...
if [ "$kernel_ver" != "auto" ]; then
  print_info "built with kernel version: $kernel_ver"
fi
...
if [ "$compress_img" ]; then
  image_zip="$data_dir/shimboot_${distro}_${board}.zip"
