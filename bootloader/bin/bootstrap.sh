...
write_shimboot_fstab() {
  local newroot_mnt="$1"
  local root_dev="$2"
  local root_type="$3"
  local fstab_path="$newroot_mnt/etc/fstab"

  if [ ! -d "$newroot_mnt/etc" ]; then
    return 0
  fi

  # Only manage fstab for rootfs images that opted in during setup. This avoids
  # clobbering other distro fstabs while still fixing Gentoo's empty fstab.
  if [ -s "$fstab_path" ] && ! grep -q "shimboot-managed-fstab" "$fstab_path" 2>/dev/null; then
    return 0
  fi

  cat > "$fstab_path" << EOF
# shimboot-managed-fstab
# Generated by shimboot bootloader for the selected rootfs.
...
boot_target() {
  ...
  if [ -x "$(command -v cryptsetup)" ] && cryptsetup luksDump "$target" >/dev/null 2>&1; then
    ...
    write_shimboot_fstab /newroot /dev/mapper/rootfs ext4
  else
    mount $target /newroot
    write_shimboot_fstab /newroot "$target" ext4
  fi
